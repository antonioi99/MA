...
Some of the files generated by the scripts described in the Python Package 
section --- classifier predictions, explanations, similarity groups, 
and forward simulation results --- are available for direct download 
from the Zenodo repository associated with the present work:

> [https://doi.org/10.5281/zenodo.20505242](https://doi.org/10.5281/zenodo.20505242)

The repository contains the following files:

| File/Folder | Description | Generated by |
|---|---|---|
| `classification_model_predictions/` | Classifier predictions and ground truth labels for the development and test sets | `main_classification_model.py` |
| `explanations/pkl/` | Raw SHAP, LIME, and attention explanation objects | `main_explanations.py --type shap/lime/attention` |
| `explanations/NLP_format/merged_data/merged_data.json` | All explanations converted into eight verbalization formats and merged into a single file | `main_explanations.py --type formatter` and `--type merge` |
| `similarity_groups/similarity_groups.json` | Four-shot development groups for each test instance based on TF-IDF cosine similarity | `main_similarity_groups.py` |
